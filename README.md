...
| Agent deployer   | Builds + pushes image, registers Foundry agent version    | [deploy_agent.py](deploy_agent.py)      |
...
The full Framingham-style CSV is included as `data/cardio vascular_dataset.csv`.
Generate the train/inference split:

```powershell
python utility_funcs/split_cardio_dataset.py
```

This produces `data/cardio_train.csv` (≈8.5k rows, with labels) and
`data/cardio_inference.csv` (≈2.1k rows, no labels). The script reads from
and writes to `data/` regardless of where you invoke it.
...
```powershell
python utility_funcs/deploy_agent.py --acr <your-acr-name>.azurecr.io
```
...
table of cardiovascular risk factors and a `TenYearCHD` label. Please consult
...
code in this repository only; the bundled dataset retains its original Kaggle
